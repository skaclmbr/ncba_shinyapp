...
ebd_data = read.csv("input_data/ebd_no_obs.csv") #for now (testing), this is only Raleigh West SE data, connect to MDB later
head(ebd_data, 5)
...
print("cumulative plot loaded")
# function to plot new COVID cases by date
new_cases_plot = function(ebd_aggregated, plot_date) {
  plot_df_new = subset(ebd_aggregated, date<=plot_date)
  g1 = ggplot(plot_df_new, aes(x = date, y = new, colour = region)) + geom_line() + geom_point(size = 1, alpha = 0.8) +
    # geom_bar(position="stack", stat="identity") +
    ylab("New Checklists") + xlab("Date") + theme_bw() +
    scale_colour_manual(values=c(checklist_col)) +
    scale_y_continuous(labels = function(l) {trans = l / 1000000; paste0(trans, "M")}) +
    theme(legend.title = element_blank(), legend.position = "", plot.title = element_text(size=10),
          plot.margin = margin(5, 12, 5, 5))
...
print("sars cumulative plot loaded")
...
print("sars new cases plot loaded")
...
print("country cases plot loaded")
...
print("country cases cumulative plot loaded")
...
print("country cases cumulative log loaded")
...
if (any(grepl("/", ebd_data$OBSERVATION_DATE))) {
  ebd_data$OBSERVATION_DATE = format(as.Date(ebd_data$OBSERVATION_DATE, format="%d/%m/%Y"),"%Y-%m-%d")
} else { ebd_data$OBSERVATION_DATE = as.Date(ebd_data$OBSERVATION_DATE, format="%Y-%m-%d") }
ebd_data$OBSERVATION_DATE = as.Date(ebd_data$OBSERVATION_DATE)
ebd_min_date = as.Date(min(ebd_data$OBSERVATION_DATE),"%d/%m/%Y")
# current_date = as.Date(max(ebd_data$OBSERVATION_DATE),"%d/%m/%Y")
...
#
print("cv data and country data merged")
...
# creat variable for today's data
# ebd_today = subset(ebd_data, OBSERVATION_DATE==current_date)
# current_case_count = length(ebd_today$SAMPLING_EVENT_IDENTIFIER)
# current_case_count_China = sum(cv_today$cases[cv_today$country=="Mainland China"])
# current_case_count_other = sum(cv_today$cases[cv_today$country!="Mainland China"])
# current_death_count = sum(cv_today$deaths)
...
print("subset for state and today's data")
...
print("write current day's data")
...
basemap = leaflet(plot_map) %>%
  ...
  addLayersControl(
    ...
    overlayGroups = c("Checklists", "Point Counts"),
    options = layersControlOptions(collapsed = FALSE)) %>%
  hideGroup(c("Checklists", "Point Counts")) %>%
...
ebd_aggregated = aggregate(ebd_data$SAMPLING_EVENT_IDENTIFIER, by=list(Date=ebd_data$OBSERVATION_DATE), FUN=length)
...
print(head(ebd_aggregated, 5))
# ebd_aggregated$OBSERVATION_DATE = as.Date(ebd_aggregated$OBSERVATION_DATE, "%m/%d/%Y")
...
ui <- bootstrapPage(
  ...
  navbarPage(theme = shinytheme("flatly"), collapsible = TRUE,
             ...
             windowTitle = "NCBA Block Mapper",

             tabPanel("NCBA Block mapper",
                      div(class="outer",
                          ...
                          absolutePanel(id = "controls", class = "panel panel-default",
                                        ...
                                        # plotOutput("ncba_checklist_plot", height="130px", width="100%"),
                                        #
                                        # sliderTextInput("plot_date",
                                        #                 label = h5("Select mapping date"),
                                        #                 choices = format(unique(as.Date(ebd_data$OBSERVATION_DATE, "%m/%d/%Y")), "%d %b %y"),
  ...
  observeEvent(input$plot_date, {
    leafletProxy("mymap") %>%
      ...
      addCircleMarkers(data = reactive_db(), lat = ~ latitude, lng = ~ longitude, weight = 1, radius = ~(cases)^(1/5.5),
                       fillOpacity = 0.1, color = checklist_col, group = "Checklists",
